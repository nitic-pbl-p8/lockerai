...
export class OwnerLostItemsDataLoader extends BaseDataLoader<LostItem['ownerId'], LostItem[]> {
  constructor(
  ...
  protected async batchLoad(ownerIds: User['id'][]): Promise<(LostItem[] | Error)[]> {
    const lostItems = await this.lostItemRepository.findManyByOwnerIds(ownerIds);
    if (lostItems.length === 0) {
      return ownerIds.map(() => []);
    }

    const mappedLostItemsList = ownerIds.map((ownerId) => {
      const mappedLostItems = lostItems.filter((lostItem) => lostItem.ownerId === ownerId);
      if (mappedLostItems.length === 0) {
        return new Error(`Cannot find LostItem with ownerId ${ownerId}.`);
      }
